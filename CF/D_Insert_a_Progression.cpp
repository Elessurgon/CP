...
void solve() {
    ...
    REP(i, 0, 1) {
        ...
        REP(i, 0, n - 2) {
            ans = min(ans, sum - abs(a[i] - a[i + 1]) + abs(a[i] - x) + abs(a[i + 1] - 1) + x - 1);
        }
        ans = min(ans, sum + abs(a[n - 1] - x) + x - 1);

        ll mn = abs(a[0] - 1);
        REP(i, 0, n - 2) {
            ans = min(ans, sum - abs(a[i] - a[i + 1]) + mn + abs(a[i + 1] - x) + abs(a[i] - x));
            mn = min(mn, 0LL - abs(a[i] - a[i + 1]) + abs(a[i] - 1) + abs(1 - a[i + 1]));
        }
        ans = min(ans, sum + mn + abs(a[n - 1] - x));
        reverse(all(a));
    }
    cout << ans << endl;
}
