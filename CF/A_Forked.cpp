...
void solve() {
    ...
    set<pair<ll, ll>> k, q;
    REP(i, 0, 3) {
        k.insert(make_pair(kx + a * dx[i], ky + b * dy[i]));
        k.insert(make_pair(kx + b * dx[i], ky + a * dy[i]));

        q.insert(make_pair(qx + a * dx[i], qy + b * dy[i]));
        q.insert(make_pair(qx + b * dx[i], qy + a * dy[i]));
    }
