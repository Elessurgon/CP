...
bool check(ll m, vl a, vl b, ll n, ll k) {
    ll re = 0;
    ll ans = 0;
    REP(i, 0, n - 1) {
        if (m <= a[i]) ans += m;
...
void solve() {
    ...
    ll ans = 0;
    REP(i, 0, n - 1) {
        auto bs = lower_bound(all(b), a[i]);
        ll nums = b.end() - bs;
        auto as = lower_bound(all(a), a[i]);
        ll revs = nums - (a.end () - as);
        // DBG(revs, nums, "a", a[i]);
        if (revs <= k) {
    ...
    REP(i, 0, n - 1) {
        auto bs = lower_bound(all(b), b[i]);
        ll nums = b.end() - bs;
        auto as = lower_bound(all(a), b[i]);
        ll revs = nums - (a.end () - as);
        // DBG(revs, nums, "b", b[i]);
        if (revs <= k) {
            ans = max(ans, nums * b[i]);
        }
    }
    cout << ans << endl;
}
