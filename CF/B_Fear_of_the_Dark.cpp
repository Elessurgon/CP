...
bool check(double w) {
    ...
    bool PinA = 1e-6 <= (w * w - ((px - ax) * (px - ax) + (py - ay) * (py - ay)));
    bool PinB = 1e-6 <= (w * w - ((px - bx) * (px - bx) + (py - by) * (py - by)));
    bool OinA = 1e-6 <= (w * w - ((ox - ax) * (ox - ax) + (oy - ay) * (oy - ay)));
    bool OinB = 1e-6 <= (w * w - ((ox - bx) * (ox - bx) + (oy - by) * (oy - by)));
    bool intersect = 1e-6 <= ((2.0 * w) - sqrt((ax - bx) * (ax - bx) + (ay - by) * (ay - by)));

    if ((PinA && OinA) || (PinB && OinB)) {
        return true;
    } else if (intersect && ((PinA && OinB) || (PinB && OinA))) {
        return true;
    }
    return false;
}
