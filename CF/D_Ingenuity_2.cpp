...
bool exr = false, exh = false;

bool f(pl& r, pl& h, pl d) {
    pl ar = {r.f + d.f, r.s + d.s}, ah = {h.f + d.f, h.s + d.s};
    ll dr = abs(ar.f - h.f) + abs(ar.s - h.s);
    ll dh = abs(ah.f - r.f) + abs(ah.s - r.s);
    if (dh == dr) {
        if (!exr) {
            r = ar;
            exr = true;
