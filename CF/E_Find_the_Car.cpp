...
void solve() {
    ...
    REP(i, 0, q - 1) {
        ll post;
        cin >> post;
        ll index = postToIndex[*lower_bound(all(a), post)];
        ll ans = 0;
        ...
        if (post == 0) {
            ...
            if (index != 0) {
                ans = b[index - 1] + ((post - a[index - 1]) * ((b[index] - b[index - 1]) / (a[index] - a[index - 1])));
            } else {
                ans = post * b[index] / a[index];
            }
            cout << ans << " ";
        }
    }
