...
void solve() {
    ...
    if (b <= k && a <= k) {
        ...
        ll ansb = 1e18;
        ll cost = abs(pts[i].f - pts[b].f) + abs(pts[i].s - pts[b].s);
        ansb = min(cost, ansb);
    }

    ll ansa = 1e18;
    REP(i, 0, k) {
        ll cost = abs(pts[i].f - pts[a].f) + abs(pts[i].s - pts[a].s);
        ansa = min(cost, ansa);
    }
    ll cost = abs(pts[b].f - pts[a].f) + abs(pts[b].s - pts[a].s);
    cout << min(ansa + ansb, cost) << endl;
}
else if (b > k && a <= k) {
    ll ans = 1e18;
    REP(i, 0, k) {
