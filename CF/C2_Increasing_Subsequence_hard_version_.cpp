...
void solve() {
    ...
    while (can && l <= r) {
        if (a[r] == a[l]) {
            ...
            if (state.empty() || state.back() < a[x]) {
                test1.pb(a[x]);
                x++;
                while (test1.back() < a[x] && x < n - 1) {
                    test1.pb(a[x]);
                    x++;
                }
            }

            if (state.empty() || state.back() < a[y]) {
                test2.pb(a[y]);
                y--;
                while (test2.back() < a[y] && y >= 0) {
                    test2.pb(a[y]);
                    y--;
                }
            }

            if (sz(test1) > sz(test2)) {
                REP(k, 0, sz(test1) - 1) {
                    state.pb(a[l]);
                    ans.pb('L');
                    l++;
