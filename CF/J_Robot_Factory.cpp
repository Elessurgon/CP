...
void dfs(ll i, ll j, ll &s) {
    vis[i][j] = true;
    REP(x, 0, 3) {
        ll nx = i + v[x], ny = j + h[x];
        if ((v[x] == 0 && h[x] == -1 && b[i][j] % 2 == 0) || (v[x] == 0 && h[x] == 1 && ((b[i][j] / 2) / 2) % 2 == 0) || (v[x] == -1 && h[x] == 0 && (((b[i][j] / 2) / 2) / 2) % 2 == 0) || (v[x] == 1 && h[x] == 0 && (b[i][j] / 2) % 2 == 0)) {
            if (!vis[nx][ny]) {
                s++;
                dfs(nx, ny, s);
            }
