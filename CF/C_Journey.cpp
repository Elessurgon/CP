...
void solve() {
    ...
    vector<double> probs;
    auto dfs = [&](auto&& dfs, ll u, ll p, double prob, ll dist) -> void {
        double ps;
        if (u == p || sz(a[u]) == 1) {
            ps = 1.0 / ((double)(sz(a[u])));
        } else {
            ps = 1.0 / ((double)(sz(a[u]) - 1));
        }
        // DBG(u, p, prob, dist, ps);
        for (auto v : a[u]) {
            if (v != p) {
                dfs(dfs, v, u, ps * prob, dist + 1);
            }
        }
        if (sz(a[u]) - 1 == 0) {
            probs.pb(prob * ((double)dist));
        }
    };
    dfs(dfs, 0, 0, 1.0, 0);
    // DBGvec(probs);
