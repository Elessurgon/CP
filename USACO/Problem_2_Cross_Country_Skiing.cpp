...
void solve() {
    ll m, n;
    cin >> m >> n;
    vvl a(m, vl(n)), r(m, vl(n)), d(m, vl(n, 1e18));
    vvb vis(m, vb(n));
    ...
    REP(i, 0, m - 1) {
        REP(j, 0, n - 1) {
            cin >> r[i][j];
            if (r[i][j] == 1) ps.pb({i, j});
        }
    }

    auto dfs = [&](auto&& dfs, ll i, ll j, ll cst) -> void {
        vis[i][j] = true;
        REP(p, 0, 3) {
            ll x = i + dx[p], y = j + dy[p];
            if (x >= 0 && x < m && y >= 0 && y < n) {
                if (!vis[x][y] && abs(a[x][y] - a[i][j]) <= cst) {
                    dfs(dfs, x, y, cst);
                }
            }
        }
    ...
    auto check = [&](ll d) -> bool {
        ...
        dfs(dfs, ps[0].f, ps[0].s, d);
        bool ok = true;
        REP(i, 0, sz(ps) - 1) {
            if (!vis[ps[i].f][ps[i].s]) {
                ok = false;
            }
        }
