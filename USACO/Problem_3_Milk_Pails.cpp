...
void solve() {
    ...
    auto dfs = [&](auto&& dfs, ll newx, ll newy, ll step) -> void {
        if (step > k || vis[newx][newy][step]) return;
        // DBG(newx, newy, step);

        vis[newx][newy][step] = true;

        ans = min(ans, abs(m - (newx + newy)));
        dfs(dfs, newx, y, step + 1);
        dfs(dfs, x, newy, step + 1);

        dfs(dfs, newx, 0, step + 1);
        dfs(dfs, 0, newy, step + 1);

        ll zero = 0;
        ll lx = max(zero, newx - (y - newy));
        ll ly = max(zero, newy - (x - newx));

        dfs(dfs, lx, min(y, newx + newy), step + 1);
        dfs(dfs, min(x, newx + newy), ly, step + 1);
    };
    dfs(dfs, 0, 0, 0);
    cout << ans;
}
