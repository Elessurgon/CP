...
ll count_divisible(ll n, ll m, ll mid) {
    // DBG(lcm(n, m));
    return (mid / n) + (mid / m) - 2 * (mid / lcm(n, m));
}

ll kth_smallest_divisible(ll n, ll m, ll k) {
    ll low = 1;
    ll high = 1e19;

    while (low < high) {
        ll mid = low + (high - low) / 2;
        ll cnt = count_divisible(n, m, mid);
...
void solve() {
    ll n, m, k;
    cin >> n >> m >> k;
    cout << kth_smallest_divisible(n, m, k) << endl;
}
