...
void solve() {
	...
	vector<vector<int>> dp(l1 + 1, vector<int>(l2 + 1, 1e5));
	REP(i, 0, l1) {
		dp[i][0] = i;
	}
	REP(i, 0, l2) {
	...
	REP(i, 1, l1) {
		REP(j, 1, l2) {
			int mismatch = min(dp[i - 1][j] + 1, min(dp[i][j - 1] + 1, dp[i - 1][j - 1] + 1));
			int match = min(dp[i - 1][j] + 1, min(dp[i][j - 1] + 1, dp[i - 1][j - 1]));
			if(s1[i - 1] == s2[j - 1]) {
				dp[i][j] = match;
			} else {
				dp[i][j] = mismatch;
