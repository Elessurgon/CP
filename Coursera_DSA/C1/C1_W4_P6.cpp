...
double minimumDistance(int left, int right) {
	...
	if (left + 1 == right) {
		double dist = pow(pts[left].f - pts[right].f, 2) + pow(pts[left].s - pts[right].s, 2);
		return dist;
	}
	double dist1 = minimumDistance(left, mid);
	double dist2 = minimumDistance(mid + 1, right);

	double d = min(dist1, dist2);
	vector<pair<int, int>> midpts;

	REP(i, left, right) {
		if ((pts[mid].f - sqrt(d) - pts[i].f <= 0 && 0 <= pts[mid].f + sqrt(d) - pts[i].f)) {
			midpts.pb({pts[i].f, pts[i].s});
		}
	}	
	sort(all(midpts), cmp);
	
	double stripDist = 1e19;
	REP(i, 0, sz(midpts) - 1) {
		REP(j, i + 1, min(sz(midpts), i + 1 + 8) - 1) {
			stripDist = min(stripDist, pow(midpts[i].f - midpts[j].f, 2) + pow(midpts[i].s - midpts[j].s, 2));
		}		
	}
	double ans = min(stripDist, d);
	return ans;
}
...
double bruteForce() {
	...
	REP(i, 0, sz(pts) - 1) {
		REP(j , i + 1, sz(pts) - 1) {
			d =  min(d, pow(pts[i].f - pts[j].f, 2) + pow(pts[i].s - pts[j].s, 2));
		}
	}
	return d;
}
